...
pygame.display.set_caption("Fortess Defender")
...
if os.path.exists('score.txt'):
    with open('score.txt', 'r') as file:
        high_score = int(file.read()) #convert str to int and assign to high_ score variable
...
for enemy in enemy_types:
    ...
    for animation in animation_types:
        #reset temporary list o images
        temp_list = []
        #deine number of frames
        num_of_frames = 20
...
class Castle():
    ...
    def repair(self):
        if self.money >= 1000 and self.health < self.max_health:
            self.health += 500 #fixing castle health by adding more health
            self.money -= 1000
...
#create castle
castle = Castle(castle_image_100, castle_image_50, castle_image_25, SCREEN_WIDTH - 250, SCREEN_HEIGHT - 300, 0.2) 
...
while run: 
    ...
    if game_over == False:   
        ...
        draw_text('PRESS "SPACE" TO PLAY AGAIN', font, WHITE, 200, 360)
